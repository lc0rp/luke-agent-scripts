...
description: Long-running coding sessions (background + tmux) for Codex, Claude, OpenCode, or Pi.
...
If you must run an interactive CLI directly with `bash`, set `pty:true` so the tool gets a real TTY.
